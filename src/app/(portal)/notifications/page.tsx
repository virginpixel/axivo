    ...
    <div>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2">
        <nav aria-label="Filter by status" className="flex flex-wrap gap-2">
          <CompanyPill href={`/notifications${q ? `?q=${encodeURIComponent(q)}` : ""}`} label="All statuses" active={!params.status} />
          {STATUSES.map((status) => (
            <CompanyPill
              key={status}
              href={`/notifications?status=${status}${q ? `&q=${encodeURIComponent(q)}` : ""}`}
